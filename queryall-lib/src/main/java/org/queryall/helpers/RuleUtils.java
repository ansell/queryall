...
{
    public static final Logger log = Logger.getLogger(RuleUtils.class
            .getName());
	...
	//        final Map<URI, NormalisationRule> allNormalisationRules = this.getAllNormalisationRules();
	        ...
	        for(final URI nextProviderNormalisationRule : rdfNormalisationsNeeded)
	        {
	            ...
	            {
	            	if(Settings._DEBUG)
	            		log.debug("Could not find requested Normalisation Rule nextProviderNormalisationRule="+nextProviderNormalisationRule.stringValue());
	            }
	        }
	        
	        if(Settings._TRACE)
	        {
	            log.trace("Settings: rule sorting started");
	        }
	        ...
	        if(Settings._TRACE)
	        {
	            log.trace("Settings: rule sorting finished");
	        }
	 ...
	 */
	public static boolean runRuleTests(Collection<RuleTest> myRuleTests, Map<URI, NormalisationRule> allNormalisationRules)
	{
	    ...
	    for(final RuleTest nextRuleTest : myRuleTests)
	    {
	        ...
	        {
	            for(final NormalisationRule nextRule : getSortedRulesByUris(allNormalisationRules, nextRuleTest.getRuleUris(), SortOrder.LOWEST_ORDER_FIRST))
	            {
	            ...
	            if(nextInputTestResult.equals(nextTestOutputString))
	            {
	                ...
	                {
	                    log
	                            .debug("TEST-PASS input test pass: nextTestInputString="
	                                    + nextTestInputString
	                                    + " nextInputTestResult="
	                                    + nextInputTestResult);
	                }
	            ...
	            else
	            {
	                ...
	                if(Settings._INFO)
	                {
	                    ...
	                    log
	                            .info("TEST-FAIL: nextRuleTest.toString()="
	        ...
	        if(nextRuleTest.getStages().contains(NormalisationRuleImpl.getRdfruleStageBeforeResultsImport()))
	        {
	            ...
	            for(final NormalisationRule nextRule : getSortedRulesByUris(allNormalisationRules, nextRuleTest.getRuleUris(), SortOrder.HIGHEST_ORDER_FIRST))
	            {
	                ...
	                if(nextOutputTestResult.equals(nextTestInputString))
	                {
	                    ...
	                    {
	                        log
	                                .debug("TEST-PASS output test pass: nextTestInputString="
	                                        + nextTestInputString
	...
	public static List<NormalisationRule> getSortedRulesForProviders(Collection<Provider> providers, Map<URI, NormalisationRule> allNormalisationRules, SortOrder sortOrder)
	{
	    List<NormalisationRule> results = new LinkedList<NormalisationRule>();
	    
	    for(Provider nextProvider : providers)
	    {
	        results.addAll(getSortedRulesByUris(allNormalisationRules, nextProvider.getNormalisationUris(), sortOrder));
	    }
	    ...
	    {
	        log.error("sortOrder unrecognised sortOrder="
	                        + sortOrder);
	    }
