...
public class TemplateImpl extends Template
{
    ...
    public boolean toRdf(Repository myRepository, URI keyToUse, int modelVersion) throws OpenRDFException
    {
        ...
        try
        {
            ...
            URI templateInstanceUri = this.getKey();
            ...
            con.add(templateInstanceUri, RDF.TYPE, getTemplateTypeUri(), keyToUse);
            con.add(templateInstanceUri, ProjectImpl.getProjectCurationStatusUri(), curationStatusLiteral, keyToUse);
            con.add(templateInstanceUri, getTemplateMatchRegex(), matchRegexLiteral, keyToUse);
            con.add(templateInstanceUri, getTemplateIsNativeFunction(), isNativeFunctionLiteral, keyToUse);
            
            if(nativeFunctionUri!= null && !nativeFunctionUri.trim().equals(""))
            {
                con.add(templateInstanceUri, getTemplateNativeFunctionUri(), nativeFunctionUriLiteral, keyToUse);
            }
            
            con.add(templateInstanceUri, getTemplateTemplateString(), templateStringLiteral, keyToUse);
            con.add(templateInstanceUri, getTemplateContentType(), contentTypeLiteral, keyToUse);
            con.add(templateInstanceUri, getTemplateOrder(), orderLiteral, keyToUse);
            
            if(referencedTemplates != null)
            {
                for(URI nextReferencedTemplate : referencedTemplates)
                {
                    con.add(templateInstanceUri, getTemplateReferencedTemplate(), nextReferencedTemplate, keyToUse);
                }
            }
