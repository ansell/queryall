...
public class BlacklistEntry
{
    ...
    public void addErrorMessageForRunnable(RdfFetcherQueryRunnable errorRunnable)
    {
        ...
        resultBuffer.append("Failed query key : "+errorRunnable.getOriginalQueryBundle().getQueryType().getKey().stringValue()+"<br />\n");
        resultBuffer.append("Failure message : "+errorRunnable.getLastException().toString()+"<br />\n");
        
        resultBuffer.append("Time to fail (milliseconds) : "+(errorRunnable.getQueryEndTime().getTime()-errorRunnable.getQueryStartTime().getTime())+" <br />\n");
    ...
    public String errorMessageSummaryToString(boolean includeSpecificQueries)
    {
        ...
        resultBuffer.append("Total time taken for failed queries: ").append(Long.toString(totalTime)).append("<br />\n");
        resultBuffer.append("Total number of failed queries: ").append(Integer.toString(numberOfFailures)).append("<br />\n");
        resultBuffer.append("Average time: ").append(Double.toString((1.0*totalTime)/numberOfFailures)).append("<br />\n");
